...
typedef struct {
    ...
    double *e;       // ad e ogni thread gli andra' ad aggiungere l'errore per la posizione current_idx
...
void inizializza_grafo(grafo *g, int num_nodi, int *num_archi) {
    ...
    g->de.nodi = NULL;
    g->de.num_de = 0;
    g->N_archi = malloc(sizeof(int));
    *g->N_archi = *num_archi;
}

bool arco_valido(grafo *g, int from, int to) {

    if (from == to) return false; // return 0 se from = to
    inmap *dest = &g->in[to];
    for (int i = 0; i < dest->num_in; i++) {
        if (dest->nodi_entranti[i] == from) return false; // Controlla duplicati
    }
...
FILE *grafo_start(const char *filename, grafo *g){
    ...
    inizializza_grafo(g, r, &num_archi);
...
void DE(grafo *g) {
    ...
    int *temp = realloc(g->de.nodi, g->de.num_de * sizeof(int));
    if (temp == NULL) {
...
void *cbody(void *arg)
{  
  dati_consumatori *a = (dati_consumatori *)arg; // *a = *arg -> tipo consumatori

  //puts("consumatore partito");
  Arco n;            // Variabile per memorizzare l'arco prelevato dal buffer
  do {
    xsem_wait(a->sem_data_items,__LINE__,__FILE__); // Attesa di un elemento disponibile nel buffer e decremento il semaforo degli elementi disponibili
    xpthread_mutex_lock(a->pmutex_buf,QUI);         // Blocco del mutex prima di accedere al buffer condiviso
    n = a->buffer[*(a->pcindex) % Buf_size];        // Lettura del numero dal buffer
    *(a->pcindex) +=1;                              // aggiornamento dell'indice del consumatore
    xpthread_mutex_unlock(a->pmutex_buf,QUI);       // Sblocco del mutex dopo aver accesso al buffer
    xsem_post(a->sem_free_slots,__LINE__,__FILE__); // Incremento del semaforo degli slot liberi nel buffer
    if(n.from == -1 && n.to == -1) break;           // Controllo del valore di stop (-1), se trovato esce dal ciclo
    ...
    xpthread_mutex_lock(a->pmutex_grafo,QUI);
...
void *fun_segnale(void *arg){
    ThreadSignal *d = (ThreadSignal*)arg;
    sigset_t mask;
    sigemptyset(&mask); //inizializzaìo maschera  segnali
    sigaddset(&mask, SIGUSR1); //aggiungo SIGUSR1 a maschera segnali
    int s;
    while(*d->exit == 0){
...
void *aux_thread(void *arg){
    ...
    while (true){
        ...
        if (*(a->current_idx) >= a->g->N) {         // se l'indice supera la grandezza dell'errore
        ...
        double y = 0;
            ...
            xpthread_mutex_lock(a->pmutex_e, QUI);          // prendo il mutex dell'errore
            *(a->e) += fabs(a->x_1[local_idx] - a->x[local_idx]);   // errore locale = differenza in valore assoluto tra x_1 e x
            xpthread_mutex_unlock(a->pmutex_e, QUI);      
    }
...
double *pagerank(grafo *g, double d, double eps, int maxiter, int taux, int *numiter){
    ...
    x = malloc((g->N)*sizeof(double));           // array x di t
    if (x == NULL) {
    perror("Allocazione fallita per in");
    free(x);
    exit(EXIT_FAILURE);
    ...
    x_1 = malloc((g->N)*sizeof(double));           // array x_1 di t
    if (x_1 == NULL) {
    perror("Allocazione fallita per in");
    free(x_1);
    free(x);
    ...
    pthread_mutex_t muidx = PTHREAD_MUTEX_INITIALIZER;
    sem_t sem_free_slots, sem_data_items;
    sem_t sem_work_to_do, sem_work_done;
    ...
    ThreadSignal signal_thread_data = {
    .N = g->N,
    .x = x,
    .exit = &t_exit,
    .iter = numiter
...
int main(int argc, char *argv[]){
    ...
    grafo g;
    FILE *f = grafo_start(argv[optind],&g);

    // buffer produttori-consumatori
    Arco buffer[Buf_size];
    int pindex=0, cindex=0;
    pthread_mutex_t mucbuf = PTHREAD_MUTEX_INITIALIZER;
    pthread_mutex_t mucgrafo = PTHREAD_MUTEX_INITIALIZER;
    pthread_mutex_t muagrafo = PTHREAD_MUTEX_INITIALIZER;
    sem_t sem_free_slots, sem_data_items;
    ...
    pthread_t prod;           // id thread produttori
    ...
    // creo il produttore
    pthread_create(&prod, NULL, pbody, (void *)&ap);
    ...
    xpthread_mutex_lock(&mucbuf, QUI);
    buffer[pindex % Buf_size] = (Arco){-1, -1};
    pindex++;
    xpthread_mutex_unlock(&mucbuf, QUI);
    ...
    double *arr_page_rank = pagerank(&g, d, e , m, t, &numiter);

    stampa_page_rank(arr_page_rank,g.N,numiter,k);
